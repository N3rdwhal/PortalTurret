...
Servo base;
int armRot = 30; //creates variable for servo rotation (int = var)
int baseRot = 90;  //creates variable for servo rotation in base
...
void setup() {
  ...
  pinMode(6, OUTPUT);  //attach a set of bullet LEDs to pin and set it to an output
  pinMode(12, OUTPUT);
  ...
  targetAcquired = digitalRead(5);
    if (targetAcquired == LOW) {
      ...
      if (turretStatus == 1) {
        ...
        for (baseRot = 90; baseRot < 30; baseRot--) {
          base.write(baseRot);
          delay(5);
        }
        for (baseRot = 30; baseRot > 150; baseRot++) {
          base.write(baseRot);
          delay(5);
        }
        for (baseRot = 150; baseRot < 90; baseRot--) {
          base.write(baseRot);
          delay(5);
        }
        delay(2000);
        ...
        for (armRot = 130; armRot > 0; armRot--) { //goes from 180 degress to 0, changes rotation by -1
    ...
    if (turretStatus == 0) {
      ...
      for (armRot = 0; armRot < 130; armRot++) { //goes from 0 degrees to 180, changes rotation by 1
         arms.write(armRot);
         delay(5); //waits 5 miliseconds
      }
      turretStatus = 1;
      //FIRE BULLET LEDs
      for (bullets = 0; bullets < 5; bullets++) {
        turretSounds.play(2); //play 0002 gunshots
        digitalWrite(9, HIGH);  //turn on upper LEDs
        delay(50);
        digitalWrite(9, LOW);  //turn off upper LEDs
        delay(15);
        digitalWrite(10, HIGH); //turn on lower LEDs
        delay(50);
        digitalWrite(10, LOW);  //turn off lower LEDs
        delay(15);
        digitalWrite(6, HIGH);  //turn on upper LEDs
        delay(50);
        digitalWrite(6, LOW);  //turn off upper LEDs
        delay(15);
        digitalWrite(12, HIGH);  //turn on upper LEDs
        delay(50);
        digitalWrite(12, LOW);  //turn off upper LEDs
        delay(80);
      }
    }
  }
